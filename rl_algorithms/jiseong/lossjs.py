...
from rl_algorithms.registry import LOSSJS
...
class IQNLossjs:
    def __call__(
        ...
        states_ = states, state_info
        next_states_ = next_states, next_state_info
        ...
        with torch.no_grad():
            ...
            next_actions = model(next_states_).argmax(dim=1)  # double Q
            ...
            target_quantile_values, _ = target_model.forward_(
                next_states_, head_cfg.configs.n_tau_prime_samples
            )
        ...
        quantile_values, quantiles = model.forward_(
            states_, head_cfg.configs.n_tau_samples
        )
...
class C51Lossjs:
    def __call__(
        ...
        with torch.no_grad():
            ...
            next_actions = model.forward_(next_states_)[1].argmax(1)

            next_dist = target_model.forward_(next_states_)[0]
        ...
        dist, q_values = model.forward_(states_)
        log_p = torch.log(
...
class DQNLossjs:
    def __call__(
        ...
        # it resamples noisynet parameters on online network when using double q
        # but we don't because there is no remarkable difference in performance.
        next_q_values = model(next_states_)

        next_target_q_values = target_model(next_states_)
