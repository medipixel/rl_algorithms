...
from rl_algorithms.gaifo.networks import Discriminator
from rl_algorithms.gaifo.utils import gaifo_iter
from rl_algorithms.gail.learner import GAILPPOLearner
...
class GAIfOLearner(GAILPPOLearner):
    ...
    def __init__(
        ...
        head.discriminator.configs.action_size = state_size
    ...
    def update_model(self, experience: TensorTuple, epsilon: float) -> TensorTuple:
        ...
        states, actions, rewards, values, log_probs, next_states, masks = experience
        ...
        for (
            ...
            # discriminator loss
            demo_state, demo_next_state = self.demo_memory.sample(len(state))
            exp_score = torch.sigmoid(self.discriminator.forward((state, next_state)))
            demo_score = torch.sigmoid(
                self.discriminator.forward((demo_state, demo_next_state))
            )
    ...
    def save_params(self, n_episode: int):
        ...
        super()._save_params(params, n_episode)
