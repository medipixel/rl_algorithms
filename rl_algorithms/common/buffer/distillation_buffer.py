...
class DistillationBuffer:
    """Class for managing reading and writing of distillation data.
       Distillation data is stored in the distillation_dataset_path location on the actual hard disk.
    ...
    Attributes:
        ...
        distillation_dataset_path (list): list of distillation buffer path
    ...
    def __init__(
        self, batch_size: int, distillation_dataset_path: str, curr_time: str,
    ):
        ...
        self.distillation_dataset_path = distillation_dataset_path
        self.idx = 0
        self.buffer_size = 0
    ...
    def reset_dataloader(self):
        ...
        """
        dataset = DistillationDataset(self.distillation_dataset_path)
...
class DistillationDataset(Dataset):
    ...
    Attributes:
        distillation_dataset_path (str): distillation buffer path

    """

    def __init__(self, distillation_dataset_path: List[str]):
        """Initialize a DistillationBuffer object.
        ...
        sum_data_len = 0
        for _dir in self.distillation_dataset_path:
            tmp = os.listdir(_dir)
        ...
        if sum_data_len == len(self.distillation_dataset_path):
